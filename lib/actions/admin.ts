...
import { getRemainingReservationHours, getMonthlyReservationHours } from '@/lib/reservationHours'
import type { ReservationHoursSummary } from '@/lib/reservationHours'
import { getKstNow } from '@/lib/utils'
...
// 관리자가 특정 이용자(예약 상세 등에서)를 확인할 때 "현재 남아있는 예약 시간"과 "이번 달 이용 시간"을
// 함께 보여주기 위한 조회. 9시간 제한 판정(createReservation)과 동일한 계산 로직을 그대로 재사용.
export async function getUserRemainingHours(student_id: string, phone: string): Promise<ReservationHoursSummary> {
  const session = await getAdminSession()
  if (!session) return { remaining: 0, monthly: 0 }
